...
class Station(object):
    ...
    def generate_report(self):
        ...
        if self.num_initial_transmits > 0:
            self.mean_retries = float(
                self.num_retries)/self.num_initial_transmits
    ...
    def transmit(self, name):
        ...
        while not success:
            ...
            if frame.retry:
                ...
                self.busy_time = self.busy_time + \
                    (self.env.now - transmit_time)
                success = True
...
if __name__ == '__main__':
    ...
    for r in range(NUM_REPLICATIONS):
        ...
        poisson_mean = 10
        stations = [Station(env, 'Station %d' % i, exponential_mean,
                            poisson_mean) for i in range(NUM_STATIONS)]
        env.run(until=TERMINATE_TIME)
        print("Report for each Station:")
        for station in stations:
            station.generate_report()
        generate_report_single_replication(
            mean_transmit_times, mean_num_retries, channel_utilizations, stations)
    generate_report_all_replications(
        mean_transmit_times, mean_num_retries, channel_utilizations)
